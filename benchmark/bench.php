 ...
 * @author Weixian Ding <waxmoth@qq.com>
 * @version 1.3
...
class Benchmark
{
    ...
    /**
     * The current timing batch number, is incremented by start on each iteration
     * @var int
     */
    ...
    new Benchmark('Math Test', function ($iterations = 150000) {
        ...
        foreach ($functions as $key => $function) {
            if (!function_exists($function)) {
                echo "Not exist function:" . $function . "\n";
                unset($functions[$key]);
            }
        }
        unset($key);
...
cli_line("PHP Version: " . PHP_VERSION . ' // OS Version: ' . PHP_OS . ' (' . php_uname('m') . ')');
cli_line();
